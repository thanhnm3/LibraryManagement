...
const getEditPath = (id) => `/admin/books/${id}/edit`
    ...
    <div v-else class="mt-6 overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200 rounded-clay border border-slate-200">
        <thead class="bg-slate-50">
          <tr>
            <th scope="col" class="admin-book-list-view_th px-4 py-3 text-left font-body text-sm font-medium text-slate-700">
              ID
            </th>
            <th scope="col" class="admin-book-list-view_th px-4 py-3 text-left font-body text-sm font-medium text-slate-700">
              Title
            ...
            <th scope="col" class="admin-book-list-view_th px-4 py-3 text-right font-body text-sm font-medium text-slate-700">
              Actions
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-200 bg-white">
          <tr v-for="book in books" :key="book.id" class="admin-book-list-view_row">
            <td class="admin-book-list-view_td px-4 py-3 font-body text-sm text-slate-900">
              {{ book.id }}
            </td>
            <td class="admin-book-list-view_td max-w-xs truncate px-4 py-3 font-body text-sm text-slate-900">
              {{ book.title }}
            </td>
            <td class="admin-book-list-view_td px-4 py-3 font-body text-sm text-slate-600">
              {{ book.isbn }}
            </td>
            <td class="admin-book-list-view_td px-4 py-3 font-body text-sm text-slate-600">
              {{ book.publicationYear }}
            </td>
            <td class="admin-book-list-view_td max-w-xs truncate px-4 py-3 font-body text-sm text-slate-600">
              {{ authorNames(book) }}
            </td>
            <td class="admin-book-list-view_td px-4 py-3 text-right">
              <RouterLink
                :to="getEditPath(book.id)"
              ...
              >
                {{ deletingId === book.id ? 'Deleting…' : 'Xóa' }}
              </button>
            </td>
          </tr>
        </tbody>
