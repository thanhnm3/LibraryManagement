...
const loadCategories = async () => {
  ...
  errorMessage.value = ''
  try {
    const res = await getAllCategories(page.value, pageSize)
    categories.value = res.content || []
    totalPages.value = res.totalPages ?? 0
  } catch (err) {
    errorMessage.value = err.data?.message || err.message || 'Failed to load categories.'
    console.error(err)
    categories.value = []
  } finally {
...
const getEditPath = (id) => `/admin/categories/${id}/edit`
    ...
    <div v-else class="mt-6 overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200 rounded-clay border border-slate-200">
        <thead class="bg-slate-50">
          <tr>
            <th scope="col" class="admin-category-list-view_th px-4 py-3 text-left font-body text-sm font-medium text-slate-700">
              ID
            </th>
            <th scope="col" class="admin-category-list-view_th px-4 py-3 text-left font-body text-sm font-medium text-slate-700">
              Name
            ...
            <th scope="col" class="admin-category-list-view_th px-4 py-3 text-right font-body text-sm font-medium text-slate-700">
              Actions
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-200 bg-white">
          <tr v-for="cat in categories" :key="cat.id" class="admin-category-list-view_row">
            <td class="admin-category-list-view_td px-4 py-3 font-body text-sm text-slate-900">
              {{ cat.id }}
            </td>
            <td class="admin-category-list-view_td px-4 py-3 font-body text-sm text-slate-900">
              {{ cat.name }}
            </td>
            <td class="admin-category-list-view_td max-w-xs truncate px-4 py-3 font-body text-sm text-slate-600">
              {{ cat.description ?? '—' }}
            </td>
            <td class="admin-category-list-view_td px-4 py-3 text-right">
              <RouterLink
                :to="getEditPath(cat.id)"
              ...
              >
                {{ deletingId === cat.id ? 'Deleting…' : 'Xóa' }}
              </button>
            </td>
          </tr>
        </tbody>
